...
if __name__ == "__main__":
    ...
    named, _ = getopt(sys.argv[1:],
                      "hi:o:c:t:b:s:a:",
                      ["help", "input=", "output=", "chain-id=", "time=", "block=", "speed=", "adam="])
    for k, v in named:
        if k in ("-h", "--help"):
            print('''
...
Usage:
    ...
    patch-genesis [-i|--input <filename>] [-o|--output <filename>] [-c|--chain-id <chain ID>] [-t|--time <genesis time>]
        [-b|--block <initial height>] [-s|--speed <time quotient>] [-a|--adam <SDK address>]
...
Options:
    ...
    -a, --adam <SDK address>
        If set, this will be the only active validator, all others will be switched off gracefully.
        ''')
            sys.exit(1)
