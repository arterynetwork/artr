...
func validateSigners(i interface{}) error {
	...
	if !ok { return fmt.Errorf("invalid VPN traffic signers parameter type: %T", i) }

	if len(val) == 0 { return fmt.Errorf("empty VPN traffic signer list") }
	for i, s := range val {
		if s.Empty() { return fmt.Errorf("empty VPN traffic signer address (#%d)", i) }
	}
