...
type CompanyAccounts struct {
	TopReferrer     sdk.AccAddress `json:"top_referrer"`
	ForSubscription sdk.AccAddress `json:"for_subscription"`
	PromoBonuses    sdk.AccAddress `json:"promo_bonuses"`
	StatusBonuses   sdk.AccAddress `json:"status_bonuses"`
	LeaderBonuses   sdk.AccAddress `json:"leader_bonuses"`
	ForDelegating   sdk.AccAddress `json:"for_delegating"`
}

type NetworkAward struct {
	Network [10]util.Fraction `json:"network"`
	Company util.Fraction     `json:"company"`
}
...
type Params struct {
	...
	DelegatingAward   NetworkAward    `json:"delegating_award"`
	SubscriptionAward NetworkAward    `json:"subscription_award"`
}
...
func (p Params) Validate() error {
	if err := validateCompanyAccounts(p.CompanyAccounts); err != nil { return err }
	if err := validateNetworkAward(p.DelegatingAward); err != nil { return nil }
	if err := validateNetworkAward(p.SubscriptionAward); err != nil { return nil }
	return nil
}
...
func validateNetworkAward(i interface{}) error {
	...
	if na.Company.IsNegative() { return fmt.Errorf("company award must be non-negative") }
	total := na.Company
	for i := 0; i < 10; i++ {
		if na.Network[i].IsNegative() { return fmt.Errorf("level %d award must be non-negative", i+1) }
		total = total.Add(na.Network[i])
	}
	if total.GTE(util.Percent(100)) { return fmt.Errorf("total network award must be less than 100%%") }
	return nil
}
